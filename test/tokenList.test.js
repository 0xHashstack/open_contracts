...
describe("testing tokenList", async () => {
  ...
  describe("Token List", async () => {
    ...
    it("Primary Market Removal:", async () => {
      await expect(tokenList.removeMarketSupport(symbolBtc)).emit(tokenList, "MarketSupportRemoved");
      expect(tokenList.isMarketSupported(symbolBtc)).to.be.reverted;
    });

    it("Add Primary Market", async () => {
      await expect(tokenList.connect(accounts[1]).addMarketSupport(symbolBtc, 6, rets["tBtcAddress"], 100000000)).to.be
        .reverted;
      await expect(tokenList.addMarketSupport(symbolBtc, 6, rets["tBtcAddress"], 100000000)).emit(
        tokenList,
        "MarketSupportAdded",
      );
      expect(await tokenList.isMarketSupported(symbolBtc)).to.equal(true);
    ...
    it("Update Primary Market", async () => {
      await expect(tokenList.connect(accounts[1]).updateMarketSupport(symbolBtc, 8, rets["tBtcAddress"], 100000000)).to
        .be.reverted;
      await expect(tokenList.updateMarketSupport(symbolBtc, 8, rets["tBtcAddress"], 100000000)).emit(
        tokenList,
    ...
    it("Secondary Market Removal", async () => {
      await expect(tokenList.connect(accounts[1]).removeMarket2Support(symbolCAKE)).to.be.reverted;
      await expect(tokenList.removeMarket2Support(symbolCAKE)).emit(tokenList, "Market2Removed");
      expect(tokenList.isMarketSupported(symbolCAKE)).to.be.reverted;
    });

    it("Add Secondary Market", async () => {
      await expect(tokenList.connect(accounts[1]).addMarket2Support(symbolCAKE, 6, rets["tCakeAddress"])).to.be
        .reverted;
      await expect(tokenList.addMarket2Support(symbolCAKE, 6, rets["tCakeAddress"])).emit(tokenList, "Market2Added");
      expect(await tokenList.isMarket2Supported(symbolCAKE)).to.equal(true);
    });

    it("Update Secondary Market:", async () => {
      await expect(tokenList.connect(accounts[1]).updateMarket2Support(symbolCAKE, 8, rets["tCakeAddress"])).to.be
        .reverted;
      await expect(tokenList.updateMarket2Support(symbolCAKE, 8, rets["tCakeAddress"])).emit(
        tokenList,
        "Market2Updated",
      );
      expect(await tokenList.isMarket2Supported(symbolCAKE)).to.equal(true);
    ...
    it("Get Primary Market Address:", async () => {
      expect(await tokenList.getMarketAddress(symbolBtc)).to.equal(rets["tBtcAddress"]);
    });

    it("Get Primary Market Decimal:", async () => {
      expect(await tokenList.getMarketDecimal(symbolBtc)).to.equal(BigNumber.from(8));
    });
    ...
    it("Get Secondary Market Decimal:", async () => {
      expect(await tokenList.getMarket2Decimal(symbolCAKE)).to.equal(BigNumber.from(8));
    });
    ...
    it("Pause:", async () => {
      ...
      await expect(tokenList.connect(accounts[1]).pauseTokenList()).to.be.reverted;

      await expect(accessRegistry.addAdminRole(adminTokenList, accounts[1].address)).emit(
        accessRegistry,
        "AdminRoleDataGranted",
      );

      await expect(accessRegistry.addAdminRole(adminTokenList, accounts[1].address)).to.be.reverted;

      await tokenList.connect(accounts[1]).pauseTokenList();
      expect(await tokenList.isPausedTokenList()).to.equal(true);

      await expect(accessRegistry.removeAdminRole(adminTokenList, accounts[1].address)).emit(
        accessRegistry,
        "AdminRoleDataRevoked",
      );

      await expect(tokenList.connect(accounts[1]).unpauseTokenList()).to.be.reverted;
      expect(await tokenList.isPausedTokenList()).to.equal(true);
