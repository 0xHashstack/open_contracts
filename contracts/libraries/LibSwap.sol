...
library LibSwap {
	...
	function _swap(bytes32 _fromMarket, bytes32 _toMarket, uint256 _fromAmount, uint8 _mode) internal returns (uint256) {
...
// https://github.com/pancakeswap/pancake-document/blob/c3531149a4b752a0cfdf94f2d276ac119f89774b/code/smart-contracts/pancakeswap-exchange/router-v2.md#swapexacttokensfortokens
		uint[] memory ret;
		ret = IPancakeRouter01(LibCommon.PANCAKESWAP_ROUTER_ADDRESS).swapExactTokensForTokens(_fromAmount,_getAmountOutMin(addrFromMarket, addrToMarket, _fromAmount, _mode),path,address(this),block.timestamp+15);
	...
	function _getAmountOutMin(
		...
		// same length as path
		uint[] memory amountOutMins = IPancakeRouter01(LibCommon.PANCAKESWAP_ROUTER_ADDRESS).getAmountsOut(
				_amountIn,
				path
		);	
