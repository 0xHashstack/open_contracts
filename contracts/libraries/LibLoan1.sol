...
library LibLoan1 {
	...
	// emit FairPriceCall(ds.requestEventId++, _collateralMarket, _collateralAmount);
		...
		uint rF = LibReserve._getReserveFactor()*LibReserve._avblMarketReserves(_loanMarket);
		...
		if (LibReserve._avblMarketReserves(_loanMarket) >= amount + 3*LibReserve._avblMarketReserves(_loanMarket)/4)    {
				loanToCollateral = 3;
		} else     {
				loanToCollateral = 2;
		}
